const { bindPhone } = require('../../../utils/auth')
const { phoneLogin } = require('../../../utils/api/public')
const { store } = require('../../../store/app')

Page({
  data: { phone: '', code: '', loading: false, codeDisabled: false, codeText: '获取验证码', authKey: '' },
  onLoad(options) { this.setData({ authKey: options.key || '' }) },
  onGetPhoneNumber(e) {
    if (!e.detail.code) return wx.showToast({ title: '授权失败', icon: 'none' })
    this.setData({ loading: true })
    bindPhone(e.detail.code, this.data.authKey).then(() => {
  ...
  onPhoneLogin() {
    const { phone, code, authKey } = this.data
    if (!phone || !code) return wx.showToast({ title: '请填写完整', icon: 'none' })
    const data = { phone, captcha: code }
    if (authKey) data.key = authKey
    phoneLogin(data).then(data => {
