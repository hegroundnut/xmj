...
CREATE TABLE IF NOT EXISTS `eb_teaching_category` (
  ...
  `type` tinyint(1) NOT NULL DEFAULT 1 COMMENT '1=案例分类 2=课程分类 3=产品分类',
  `sort` int(11) NOT NULL DEFAULT 0 COMMENT '排序，越大越靠前',
  `status` tinyint(1) NOT NULL DEFAULT 1 COMMENT '1=启用 0=禁用',
  `add_time` int(11) NOT NULL DEFAULT 0 COMMENT '创建时间戳',
  PRIMARY KEY (`id`),
  KEY `idx_type_status` (`type`, `status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='教学分类（案例/课程/产品）';
...
-- 产品表新增分类字段
ALTER TABLE `eb_product_info` ADD COLUMN `category_id` int(11) NOT NULL DEFAULT 0 COMMENT '分类ID' AFTER `title`;
